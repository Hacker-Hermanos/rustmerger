...
impl log::Log for Logger {
    ...
    fn log(&self, record: &Record) {
        if self.enabled(record.metadata()) { // Check if the log message should be logged
            ...
            if let Some(log_file) = &self.log_file {
                if let Ok(mut file) = log_file.lock() { // Lock the file for thread-safe access
            ...
            if record.level() == Level::Error {
                if let Some(error_file) = &self.error_file {
                    if let Ok(mut file) = error_file.lock() { // Lock the file for thread-safe access
